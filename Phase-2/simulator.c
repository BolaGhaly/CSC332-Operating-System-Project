    ...
    for (i = 0; i < current_process->exe_size; ++i)
    {
        ...
        case APP:
        {
            ...
            if (current_process->execution_flow[i].strArgs1 == START)
            {
                ...
                sprintf(value, "OS: Process %d set to RUNNING state from READY state.", running_process_identifier);
            ...
            else if (current_process->execution_flow[i].strArgs1 == END)
            {
                timer(current_process->execution_flow[i].intArg2 * configDataType->quantumCycles);
                char *value = (char *)malloc(sizeof(char) * 1000);
                sprintf(value, "OS: Process %d set to EXITED state from RUNNING state.", running_process_identifier);
                output(value, configDataType);
                free(value);
        ...
        {
            char *value = (char *)malloc(sizeof(char) * 1000);
            sprintf(value, "Process: %d device input %s start, time: %d ms.", running_process_identifier, current_process->execution_flow[i].origin->strArg1, current_process->execution_flow[i].intArg2 * configDataType->quantumCycles * configDataType->ioCycleRate);
            output(value, configDataType);
            timer(current_process->execution_flow[i].intArg2 * configDataType->quantumCycles * configDataType->ioCycleRate);
            sprintf(value, "Process: %d device input %s end.", running_process_identifier, current_process->execution_flow[i].origin->strArg1);
            output(value, configDataType);
            free(value);
            break;
        }
        case DEVOUT:
        {
            ...
            output(value, configDataType);
            timer(current_process->execution_flow[i].intArg2 * configDataType->quantumCycles * configDataType->ioCycleRate);
            sprintf(value, "Process: %d device output %s end.", running_process_identifier, current_process->execution_flow[i].origin->strArg1);
            output(value, configDataType);
            free(value);
            break;
        }
        ...
        {
            char *value = (char *)malloc(sizeof(char) * 1000);
            sprintf(value, "Process: %d CPU Process, time: %d ms.", running_process_identifier, current_process->execution_flow[i].intArg2 * configDataType->quantumCycles);
            output(value, configDataType);
            timer(current_process->execution_flow[i].intArg2 * configDataType->quantumCycles);
...
void runSim(ConfigDataType *configPtr, OpCodeType *metaDataMasterPtr)
{
    ...
    Boolean simRun = True;
    OpCodeType *ptr = metaDataMasterPtr;
    process_t *prevProcess = NULL;
    OpCodeType *app_start, *app_end;
    int range = 0, process_cnt = 0;
    ...
    while (ptr != NULL)
    {
        ...
        else if (compareString(ptr->command, "app") == 0)
        {
            ...
            else if (compareString(ptr->strArg1, "end") == 0)
            {
                range++;
                app_end = ptr;
                process_list->execution_flow = (executable_t *)malloc(sizeof(executable_t) * (range + 7));
    ...
    output("OS: System End", configPtr);
