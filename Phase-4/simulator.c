...
process_t *schedule(ready_queue_t *ready_q, ConfigDataType *configPtr) {
    if (rq_size(ready_q) == 0) return NULL;
...
void print_interrupt(sim_t *sim, ConfigDataType *config) {
    ...
    if (rq_size(sim->interrupt_q) > 0) {
        ...
        sprintf(value, "OS: Interrupted by process %d, %s %sput operation", current_process->pid, current_process->execution_flow[current_process->cur_exe].origin->strArg1,
...
void runSim(ConfigDataType *configPtr, OpCodeType *metaDataMasterPtr) {
    ...
    int range = 0, process_cnt = 0, command_index;
...
void initial_process(int non, ConfigDataType *configPtr) {
    char value[BUFSIZ];
    for (int index = 0; index < non; ++index) {
